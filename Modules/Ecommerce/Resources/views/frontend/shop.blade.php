            ...
            <div class="product-grid mar-bot-30 row">
                ...
                <div class="col-sm-2 text-center mt-3">
                    <a href="{{ url('/shop') }}/{{$cat->slug}}">
                        @if($cat->icon!==null)
                            <img loading="lazy" class="category-img" data-src="{{ asset('public/images/category/icons/') }}/{{ $cat->icon }}" alt="{{ $cat->name }}">
                        @else
                ...
                <div class="col-sm-2 text-center mt-3">
                    <a href="{{ url('/shop') }}/{{$category->slug}}">
                        @if($category->icon!==null)
                            <img loading="lazy" class="category-img" data-src="{{ asset('public/images/category/icons/') }}/{{ $category->icon }}" alt="{{ $category->name }}">
                        @else
